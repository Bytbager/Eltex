...
void users_thread(mqd_t *write_mqds) {
    ...
    while(1) {
        ...
        if (prio == 32) {
            ...
            if (mq_send(*write_mqds, message, 256, 32) == -1)
                printf("wr eror!\n");
            tmp = root;
            while (tmp != NULL) {
                if (mq_send(new_user->mqds, (char *) &tmp->user, 256, 32) == -1)
            ...
            while (tmp != NULL) {
                if (tmp->user.mqds != new_user->mqds)
                    if (mq_send(tmp->user.mqds, (char *) new_user, 256, 32) == -1)
            ...
            tmp = root;
            while (tmp != NULL) {
                mq_send(tmp->user.mqds, (char *) new_user, 256, 33);
                tmp = tmp->next;
            }
        }
    }
